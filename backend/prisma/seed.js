...
async function main() {
    ...
    for (const [categoryName, name] of services) {
        const category = await prisma.category.upsert({ where: { name: categoryName }, update: {}, create: { name: categoryName } });
        const existing = await prisma.service.findFirst({ where: { name, categoryId: category.id } });
        if (!existing)
            await prisma.service.create({ data: { serviceId: 1000 + services.indexOf([categoryName, name]), name, categoryId: category.id, rate: 1.22, minQuantity: 10, maxQuantity: 100000, averageTime: '0-24 hours', providerName: 'DEMO', description: 'Compliant provider fulfillment placeholder for local testing.' } });
    }
    ...
    for (const [question, answer] of faq)
        await prisma.fAQ.upsert({ where: { id: question }, update: {}, create: { id: question, question, answer } });
}
